<template>
  <div class="auth-center">
    <div class="auth-box">
      ...
      <h2>Реєстрація</h2>
      <p class="subtitle">Створіть особистий кабінет</p>
      ...
      <div class="form">
        <div class="form-field">
          ...
          <input class="form-input" v-model="username" type="text" placeholder="Ваше ім'я" />
        </div>
        <div class="form-field">
          <label class="form-label">EMAIL</label>
          <input class="form-input" v-model="email" type="email" placeholder="your@email.com" />
        </div>
        <div class="form-field">
          <label class="form-label">ПАРОЛЬ</label>
          <input class="form-input" v-model="password" type="password" placeholder="••••••••" @keydown.enter="submit" />
        </div>

        <button class="btn-gold" @click="submit" :disabled="loading">
          <span>{{ loading ? 'ЗАВАНТАЖЕННЯ...' : 'ЗАРЕЄСТРУВАТИСЯ' }}</span>
        </button>
        ...
        <router-link to="/login" class="btn-outline">УВІЙТИ</router-link>

        <p class="switch">Вже є акаунт? <router-link to="/login">Увійти</router-link></p>
      </div>
